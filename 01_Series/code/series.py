...
richest = pd.read_csv('datasets/TopRichestInWorld.csv', usecols=['Name', 'Age'], index_col='Name').squeeze()
...
plt.xlabel('Age')
plt.title('Age Distribution of Millionaires')
plt.savefig('image/boxplot_age_distribution.png')
plt.show()

# Save the series as a csv file named: sample.csv
richest.to_csv('datasets/sample.csv', header=True)

# Read the csv you just saved
loaded_data = pd.read_csv('datasets/sample.csv', index_col='Name').squeeze()
print("\nLoaded data from the saved CSV:\n", loaded_data)

# Save the series as an Excel file named: sample.xlsx with columns 'Name' and 'Age'
richest.to_excel('datasets/sample.xlsx', index_label='Name')  # Remove header=['Name', 'Age']

# Read the Excel file you just saved
loaded_data_excel = pd.read_excel('datasets/sample.xlsx', index_col='Name').squeeze()
